...
export async function startTransactionConsumer() {
  ...
  const updateUserBalanceUsecase = new UpdateUserBalanceUsecase(new UserRepository());

  await channel.assertQueue(queue);
  
  console.log(`📥 Consumer conectado à queue '${queue}' - aguardando transações...`);
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      const { receiverid, senderid, amount } = JSON.parse(msg.content.toString());
      
      if (!receiverid || !senderid || !amount) {
        console.log("❌ Formato inválido: esperado receiverid, senderid e amount");
        return channel.nack(msg, false, false);
      }
      
      console.log(`💰 Processando transação: ${senderid} → ${receiverid} (${amount})`);
      ...
      console.log(`❌ Erro ao processar transação: ${(error as Error).message}`);
